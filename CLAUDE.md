...
- **Janela de boot (v2.2).** Após `begin()`, o `LedController` exibe os padrões de conexão/ocioso normalmente por `LED_BOOT_BLINK_MS` (5 min); depois entra em **blackout** (LEDs apagados via `allOff()` no início do ramo `!_answerActive` de `update()`) **até a 1ª resposta**. O SSE e os logs na serial **seguem ativos** no blackout — só a saída dos LEDs é suprimida. Qualquer evento `answer` (single/multiple/yesno/sequência/`test`/erro) marca `_firstAnswerReceived = true` (setado nos quatro pontos que ativam `_answerActive`: `startHold`/`startSeq`/`showTestChase`/`showError`) e **encerra o blackout em definitivo** — ele não rearma. Se a 1ª resposta chegar antes dos 5 min, o blackout nunca ocorre. O gate só roda quando não há resposta ativa, então nunca interfere na exibição de uma resposta.
